...
def cmd_update(versions):
    ...
    latest = list(
        map((lambda x: list(filter(tag_filter(x), tags))[0]), versions))
    for i in range(len(versions)):
        logging.info("Updating %s/Dockerfile with %s" %
                     (versions[i], latest[i]))
...
def cmd_build(versions, push, latest):
    ...
    for v in versions:
        ...
        if subprocess.call(['docker', 'build', '-t', "aaronngray/emcc:%s" % (v), '.']):
            logging.error("Building aaronngray/emcc:%s failed" % (v))
            os.chdir(cdir)
            continue
        logging.info("Building of aaronngray/emcc:%s finished." % (v))
        os.chdir(cdir)
        if v is latest:
            ...
            if subprocess.call(["docker",
                                "tag",
                                di,
                                "aaronngray/emcc:latest"]):

                logging.error(
                    "Tagging as latest the aaronngray/emcc:%s failed" % (v))
                continue

    if push:
        logging.info('Pushing tags aaronngray/emcc')
        push_tag("aaronngray/emcc")
...
def main():
    ...
    parser_u = subparsers.add_parser(
        'update', help='Update the Dockerfiles according to releases')
    ...
    parser_b = subparsers.add_parser(
        'build', help='Build/Push the docker images to docker repo')
